...
## [+] 大规模版本号重映射的全链路同步策略 (2026-08-07)

项目级版本号体系统一重映射（如 v0.x.x → v1.0.0）时，涉及 flow.json、plan 索引、kb 索引、dev 进度等多层文件，须按以下策略系统化执行，避免遗漏造成引用断裂：

**步骤 1：确定锚点**

- 区分"新增阶段"（v1.0.0 自身的新阶段，如 stage-01/02/03）与"历史阶段重映射"
- 确定编号锚点：按 flow.json 中 stage 现有顺序，从旧体系最早版本开始依次分配新编号
- 实例：v1.0.0 版本统一时，v0.4.2（flow.json 中最早）→ v1.0.0-stage-04，v0.5.11（flow.json 中最后）→ v1.0.0-stage-28，共 25 个旧阶段重映射

**步骤 2：全链路文件清单**

| 序号 | 文件 | 替换内容 | 替换次数（典型） |
|:--:|------|------|:--:|
| 1 | `flow.json` | stages 键名 + pipeline.current.stage + 全部 log 中的 stageName | 100+ 处 |
| 2 | `plan/index.md` | 阶段对照表（stageId 列 + 对应原版本） | ~28 行 |
| 3 | `plan/plan_log.md` | 归档条目中的版本号引用 | ~10 条 |
| 4 | `dev/current.md` | 进度表中的版本号、阶段排列 | ~30 行 |
| 5 | `kb/index.md` | 最近更新描述、版本引用 | 1~2 处 |

**步骤 3：验证方法**

- 搜索旧版本号模式（如 `v0\.`, `v0-`）在整个 `.openfeel/` 和 `plan/` 下确认无遗漏
- 排除历史日志、快照（checkpoints/）、私域用户目录——这些历史记录不应被替换
- 运行 `openfeel flow overview` 确认 28 个阶段全部状态可读

**步骤 4：版本引用与目录名的区分**

与本条目互补的「版本号重映射边界判定模式」解决的是同一重映射过程中的另一维度：区分"目录名"（组织单位，保留原名）与"版本号引用"（文本，需重映射）。本条目聚焦全链路替换的执行流程，两者共同覆盖版本号迁移的全部场景。

**关键要点：**

- **单向执行、一次性完成**：26+ 文件涉及 100+ 处替换，中途回滚成本极高——执行前在 plan_log.md 记录完整替换计划供确认
- **编号空间预留**：新增阶段独立编号（如 stage-01~03），历史阶段从预留号段之后开始（如 stage-04 起），避免未来新增阶段与历史重映射编号冲突
- **git 提交策略**：大规模替换应单次 commit 完成（`chore: 版本号统一重映射 v1.0.0`），避免分散提交导致 git history 中版本号不一致

**适用场景：**

- 项目从 v0 开发阶段进入 v1 正式版发布
- 版本号体系变更（如从短号改为长号、引入四级版本号）
- 合并多个独立版本系列到统一体系
