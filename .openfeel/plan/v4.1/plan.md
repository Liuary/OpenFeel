...
| 1.8 | **子 Agent 返回精简模式** (P2) | 为子 Agent（Executor / Reviewer / Tester / Archiver）增加"精简返回"模式：任务完成后仅输出摘要（1-2 句 + 状态标记），完整报告写入私域日志。减少 Feel 会话上下文膨胀。当前 cache 命中率 95.5% 表明影响有限，降为 P2。 |
