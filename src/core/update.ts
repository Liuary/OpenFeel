...
**快速验收行为**：运行 \`npm test\` 一次确认通过 → 检查自测报告完整性（所有自测项已执行） → 通过即通知 Feel 推进到 \`test_passed\`

## 完整验收流程

当不满足快速验收条件时执行：
1. **逐项验收**：按操作方案中的自测清单逐条验证
2. **全量测试**：运行 \`npm test\` 执行全量测试套件
...
2. **关联模块冒烟测试**：运行 \`npm test\` 中对应模块的测试用例
3. **修复涉及单元测试**：运行修复所涉函数/模块的所有单元测试

### 扩展回归

high 优先级 Bug 修复后，推荐执行全量测试套件 (\`npm test\`)。
